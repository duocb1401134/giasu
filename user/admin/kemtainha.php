...
$from = ($trang - 1) * $sotin;
// tổng số trang tin
$select = "SELECT * FROM `hocvien` WHERE ID_GS is null order by ID_HOCVIEN DESC";
            ...
            <div class="inbox-right">
                <div class="mailbox-content">
                    <table class="table">
                        <tbody>
                            <?php
                            $select = "SELECT * FROM `hocvien` WHERE `DK_TN`='1' and ID_GS is null order by ID_HOCVIEN DESC LIMIT $from,$sotin";
                            $qr = mysql_query($select);
                ...
                <nav style="text-align: center;">
                    <ul class="pagination">
                        ...
                        <li class="<?php if ($i >= $tongsotrang) echo 'disabled'; ?>">
                            <a href="index.php?p=1&i=<?php echo $i = $i + 5; //if ($i >= $tongsotrang){echo $i;} //else {echo $i = $i + 5;}     ?>&trang=<?php echo $i - 4; ?>"><span aria-hidden="true">&raquo;</span></a></li>
                    </ul>
                </nav>
